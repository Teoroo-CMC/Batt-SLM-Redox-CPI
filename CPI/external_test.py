...
def predict_cpi():
    ...
    # output
    with open(TEST_DIR / f"{input_file_name}-predictions.csv", "w") as g:
...
def print_report():
    ...
    with open(TEST_DIR / f"external_set-unseed-filters-features-predictions.csv", "r") as f:
        ...
        funct_col = titles.index("Function")
        mol_cpis = [float(l.strip("\n").split(";")[cpi_col]) for l in lines[1:]]
        mol_functs = [l.strip("\n").split(";")[funct_col] for l in lines[1:]]
